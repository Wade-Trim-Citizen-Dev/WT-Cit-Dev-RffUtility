...
:: Archive previous build if it exists (including pre-rename RFF_Merger builds)
if exist "dist\RFF_Utilities.exe" (
    echo Archiving previous build RFF_Utilities.exe to RFF_Utilities_%timestamp%.exe...
    move "dist\RFF_Utilities.exe" "dist\RFF_Utilities_%timestamp%.exe"
)
